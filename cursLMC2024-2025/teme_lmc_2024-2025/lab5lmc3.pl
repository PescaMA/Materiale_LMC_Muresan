...
/* Ordinea stricta S asociata unei ordini O, in doua moduri; alta varianta ar fi sa includem ca argument multimea A pe care sunt definite S si O si sa obtinem S ca diferenta de multimi intre O si diagonala lui A:*/

ordstrdinord(S,O) :- setof((X,Y), (member((X,Y), O), X\=Y), S), !.
ordstrdinord([],_).

ordstrictadinord([],[]).
ordstrictadinord(S,[(X,X)|T]) :- !, ordstrictadinord(S,T).
ordstrictadinord([(X,Y)|S],[(X,Y)|T]) :- ordstrictadinord(S,T).

% Ordinea O asociata unei ordini stricte S pe o multime A:

orddinordstr(O,S,A) :- diag(A,D), reuniune(S,D,O).

/* Interogati:
?- orddinordstr(O,[(a,b),(b,c),(a,c)],[a,b,c]), ordstrdinord(S,O), ordstrictadinord(Str,O).
*/

% Relatia de succesiune Succ asociata unei ordini stricte S:

succdinordstr(Succ,S) :- setof((X,Y), (member((X,Y),S),
		not((member((X,U),S), member((U,Y),S)))), Succ), !.
succdinordstr([], _).

% Relatia de succesiune Succ asociata unei ordini O:

succdinord(Succ,O) :- setof((X,Y), (member((X,Y),O), X\=Y,
	(not((member((X,U),O), X\=U, member((U,Y),O), U\=Y)))), Succ), !.
succdinord([],_).

/* Interogati:
?- S=[(a,b),(b,c),(a,c)], orddinordstr(O,[(a,b),(b,c),(a,c)],[a,b,c]), succdinordstr(Succ1,S), succdinord(Succ2,O).
?- setof(Succ, R^(relord(R,[a,b,c]), succdinord(Succ,R)), L), afislista(L), length(L,N).
