...
/* Varianta, pentru ca orice submultime a unei relatii binare antisimetrice e antisimetrica,
deci e suficient sa selectam submultimile ordinilor totale care sunt preordini, intrucat
acestea vor fi chiar ordini: */

ordpe(A,Ord) :- permutare(A,P), succlant(P,SuccLant), orddinsucc(SuccLant,A,OrdLant),
			sublista(Ord,OrdLant), preord(Ord,A).

determsucc(Succ) :- multime(A), ordpe(A,Ord), conditie(Ord), succdinord(Ord,Succ).

determSucc(Succ) :- setof(S, determsucc(S), LS), member(Succ,LS), eord(Succ).
